    ...
    <section className="py-20 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        <header className="space-y-4 text-center">
          <h1 className="text-4xl font-serif font-semibold text-gray-900">{experiences.heading}</h1>
          {experiences.intro.map((paragraph, index) => (
        ...
        <div className="grid gap-10 md:grid-cols-3">
          {experiences.tips.map((tip, index) => (
            <article key={index} className="bg-gray-50 p-8 rounded-3xl shadow-sm hover:shadow-md transition-shadow">
              <div className="h-48 mb-6 overflow-hidden rounded-2xl">
                <img
                  src={`https://source.unsplash.com/collection/1424342/${index + 1}`}
                  alt={tip.title}
                  className="w-full h-full object-cover"
                />
              </div>
              <h2 className="text-2xl font-serif font-semibold text-amber-700 mb-3">{tip.title}</h2>
